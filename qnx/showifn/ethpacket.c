...
// 8100 2044
#define ETH_PROTO_OFFSET_VLAN ETH_PROTO_OFFSET
#define VLAN_LENGTH        4
#define ETH_PROTO_VLAN     0x8100
#define VLAN               68
#define PCP                1

#define ETH_PROTO_OFFSET_IPV4  ETH_PROTO_OFFSET_VLAN + VLAN_LENGTH
#define ETH_PROTO_IPV4     0x0800
#define IPV4_PROTO_UDP     17
#define IPV4_TTL           1

#define IPV4_OFFSET        ETH_PROTO_OFFSET_IPV4 + 2
...
#define ETHLEN             (IPV4_OFFSET + MTULEN)
...
construct_packet(struct ifnet *ifp)
{
  ...
  eth_packet[ETH_DST_MAC_OFFSET + 3] = 0x7F;
  eth_packet[ETH_DST_MAC_OFFSET + 4] = 0x2A;
  eth_packet[ETH_DST_MAC_OFFSET + 5] = 0x63;
  memcpy(eth_packet + ETH_SRC_MAC_OFFSET, if_getlladdr(ifp), ETHER_ADDR_LEN);
  write_uint16(eth_packet + ETH_PROTO_OFFSET_VLAN, htons(ETH_PROTO_VLAN));
  write_uint16(eth_packet + ETH_PROTO_OFFSET_VLAN + 2, htons(0x2044));

  write_uint16(eth_packet + ETH_PROTO_OFFSET_IPV4, htons(ETH_PROTO_IPV4));

  // IPv4 Header (20 bytes)
  const uint32_t srcip = 0x00000000;
  const uint32_t dstip = 0xE37F2A63;
  ...
  write_uint16(udphdr + UDP_SRC_PORT, htons(3495));
  write_uint16(udphdr + UDP_DST_PORT, htons(3490));
  ...
  if (if_gethwassist(ifp) & CSUM_IP) {
    printf("%s: CSUM_IP HW acceleration ENABLED\n", if_name(ifp));
    m->m_pkthdr.csum_flags |= CSUM_IP;
  } else {
    printf("%s: CSUM_IP HW acceleration DISABLED\n", if_name(ifp));
  ...
  printf("%s: in_pseudo = %04x\n", if_name(ifp), udppcs);
  if (if_gethwassist(ifp) & CSUM_IP_UDP) {
    printf("%s: CSUM_IP_UDP HW acceleration ENABLED\n", if_name(ifp));
    m->m_pkthdr.csum_flags |= CSUM_IP_UDP;
    m->m_pkthdr.csum_data = UDP_CHECKSUM;
  } else {
    printf("%s: CSUM_IP_UDP HW acceleration DISABLED\n", if_name(ifp));
